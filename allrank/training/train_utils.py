...
def compute_metrics(metrics, model, dl, dev):
    ...
    for metric_name, ats in metrics.items():
        ...
        metrics_names = [
            "{metric_name}_{at}".format(metric_name=metric_name, at=at) for at in ats
        ]
...
def epoch_summary(epoch, train_loss, val_loss, train_metrics, val_metrics):
    summary = (
        "Epoch: {epoch}. Train loss: {train_loss:.6f} Val loss: {val_loss:.6f}".format(
    ...
    for metric_name, metric_value in train_metrics.items():
        summary += " Train {metric_name} {metric_value:.6f}".format(
            metric_name=metric_name, metric_value=metric_value
        )
    summary += "\n"

    for metric_name, metric_value in val_metrics.items():
        summary += " Val {metric_name} {metric_value:.6f}".format(
            metric_name=metric_name, metric_value=metric_value
        )
    summary += "\n"
...
def fit(
    ...
    for epoch in range(epochs):
        logger.info("Current learning rate is: {}".format(get_current_lr(optimizer)))
        ...
        tensorboard_metrics_dict = {
            ("train", "loss"): train_loss,
            ("val", "loss"): val_loss,
        }

        train_metrics_to_tb = {
            ("train", name): value for name, value in train_metrics.items()
        }
        tensorboard_metrics_dict.update(train_metrics_to_tb)
        val_metrics_to_tb = {
            ("val", name): value for name, value in val_metrics.items()
        }
        ...
        logger.info(
            epoch_summary(epoch, train_loss, val_loss, train_metrics, val_metrics)
        )
